...
Install it with Docker, as a bare-metal systemd service, or - for monitoring-only sites - directly on the site's UniFi gateway (about 50 MB in practice, held under a memory limit as a safety backstop so it stays clear of routing). Per-site one-liners for all three are generated in the web UI under **Settings > Multi-Site > (site) > Agents > Set up agent**. See the [Agent Deployment Guide](../src/NetworkOptimizer.Agent/README.md) for the full walkthrough, all options, and reverse-proxy configuration.
...
On ARM64 hardware (Raspberry Pi, etc.), publish with `-r linux-arm64` instead, and use the distro's protoc: the one bundled with Grpc.Tools segfaults (exit code 139) on native arm64 Linux. Install it with `sudo apt install -y protobuf-compiler` and prefix the publish command with `PROTOBUF_PROTOC=/usr/bin/protoc`. It must be protoc 3.15 or newer (`protoc --version`), which Debian 12, Raspberry Pi OS Bookworm, and Ubuntu 24.04 ship; older releases need a protoc from the [protobuf releases](https://github.com/protocolbuffers/protobuf/releases).
