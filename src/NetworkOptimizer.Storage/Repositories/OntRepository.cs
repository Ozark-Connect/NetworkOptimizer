...
public class OntRepository : IOntRepository
{
    ...
    public async Task SetOntEnabledAsync(int id, bool enabled, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            await _context.SaveChangesAsync(cancellationToken);
            _logger.LogDebug("Set ONT configuration {Id} Enabled={Enabled}", id, enabled);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to set Enabled on ONT configuration {Id}", id);
            throw;
        }
    }

    public async Task<bool> UpdateOntPollResultAsync(int id, DateTime? lastPolled, string? lastError, CancellationToken cancellationToken = default)
    {
        try
        {
        ...
        {
            _logger.LogError(ex, "Failed to update ONT poll result for {Id}", id);
            throw;
        }
    }
}
