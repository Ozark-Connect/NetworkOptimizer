...
public class LearnedCongestionProfile
{
    ...
    /// <summary>
    /// Shaper rate per Mbps of measured payload when a learned curve is deployed. Payload comes back
    /// as the tc rate that produced it at about 1.04 (TCP/IP framing against the HTB byte count);
    /// what is left is headroom below the line so the queue forms in fq_codel: 1% on PON, whose
    /// limiter is shallow, 3% on DOCSIS and DSL, 4% on shared media whose hour is an average.
    /// </summary>
    public static double ShaperFactor(ConnectionType type) => type switch
    {
        ConnectionType.Gpon or ConnectionType.XgsPon => 1.03,
        ConnectionType.DocsisCable or ConnectionType.Dsl => 1.01,
        _ => 1.00,
    };

    /// <summary>The learned shaping ceiling: the measured peak as a shaper rate.</summary>
    public static int ShapingCeiling(double measuredPeakMbps, ConnectionType type) =>
        Math.Max(1, (int)Math.Round(measuredPeakMbps * ShaperFactor(type)));
