...
public class SponsorshipService : ISponsorshipService
{
    ...
    private async Task<int> GetUsageCountInternalAsync(IServiceScope scope)
    {
        ...
        // Count audits and speed tests in parallel - use default site (first site) for sponsorship counting
        // TODO: Consider aggregating across all sites for sponsorship purposes
        const int defaultSiteId = 1001;
        var auditCountTask = auditRepository.GetAuditCountAsync(defaultSiteId);
        var speedTestCountTask = speedTestRepository.GetIperf3ResultCountAsync(defaultSiteId);
        var sqmWan1Task = speedTestRepository.GetSqmWanConfigAsync(defaultSiteId, 1);
        var sqmWan2Task = speedTestRepository.GetSqmWanConfigAsync(defaultSiteId, 2);
