...
public class AdminAuthService : IAdminAuthService
{
    ...
    private async Task RefreshCacheIfNeededAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // Reaching here means there was no stored password at all - a first run, or an
            // operator who just ran scripts/reset-password.*, which clears the row precisely
            // to force this. Either way the printed password has to become the real login, so
            // hand it to the Identity bootstrap running next: on a migrated install the admin
            // account already exists, and its own hash is what sign-in actually checks.
            _cache.PublishFirstRunPassword(generatedPassword);
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    /// Generates a secure random password (16 characters, alphanumeric, always containing a digit)
    /// </summary>
    private static string GenerateSecurePassword()
    {
        ...
        // Only 8 of the 55 characters are digits, so roughly one draw in twelve comes out with
        // none at all - which the account policy requires (RequireDigit). Redrawing until one
        // lands keeps every password uniform over the passwords that are actually acceptable,
        // where patching a digit in at a fixed position would not. Bounded so this can never
        // spin: at ~8% per draw, exhausting the attempts means a broken RNG, not bad luck.
        for (var attempt = 0; attempt < 100; attempt++)
        {
            ...
            if (Array.Exists(password, char.IsDigit))
                return new string(password);
        }

        // Unreachable in practice, but place a digit rather than hand back a password that the
        // account policy would reject - that would fail the very reset it is generated for.
        password[^1] = "23456789"[bytes[0] % 8];
        return new string(password);
    }
}
