...
public class UniFiDiscovery
{
    ...
    public async Task<List<DiscoveredDevice>> DiscoverDevicesAsync(CancellationToken cancellationToken = default)
    {
        ...
        // TEMPORARY: Add mock devices for testing device classification
        // TODO: Remove before merging to main
        discoveredDevices.AddRange(GenerateMockDevicesForTesting());
    ...
    /// <summary>
    /// TEMPORARY: Generate mock devices for testing device classification.
    /// TODO: Remove before merging to main.
    /// </summary>
    private static List<DiscoveredDevice> GenerateMockDevicesForTesting()
    {
        var mockDevices = new List<DiscoveredDevice>();
        var baseIp = 100;

        // Access Points (should appear in WiFi Optimizer)
        var apModels = new[] {
            ("U6-Pro", "U6-Pro", "uap"),
            ("U6-LR", "U6-LR", "uap"),
            ("U6-Lite", "U6-Lite", "uap"),
            ("U6-Mesh", "U6-Mesh", "uap"),
            ("U6-Enterprise", "U6-Enterprise", "uap"),
            ("U7-Pro", "U7-Pro", "uap"),
            ("UAP-AC-Pro", "UAP-AC-Pro", "uap"),
            ("UAP-AC-Lite", "UAP-AC-Lite", "uap"),
            ("UAP-IW-HD", "UAP-IW-HD", "uap"),  // In-wall with switch ports
        };

        foreach (var (model, shortname, type) in apModels)
        {
            mockDevices.Add(new DiscoveredDevice
            {
                Id = Guid.NewGuid().ToString(),
                Mac = $"aa:bb:cc:dd:{baseIp:x2}:01",
                Name = $"Mock {shortname}",
                Type = DeviceType.AccessPoint,
                HardwareType = DeviceType.AccessPoint,
                Model = model,
                Shortname = shortname,
                IpAddress = $"192.168.1.{baseIp++}",
                Firmware = "7.0.0",
                Adopted = true,
                State = 1
            });
        ...
        // Switches (should NOT appear in WiFi Optimizer)
        var switchModels = new[] {
            ("USW-24-PoE", "USW-24-PoE", "usw"),
            ("USW-16-PoE", "USW-16-PoE", "usw"),
            ("USW-Lite-8-PoE", "USW-Lite-8-PoE", "usw"),
            ("USW-Flex", "USW-Flex", "usw"),
            ("USW-Flex-Mini", "USW-Flex-Mini", "usw"),
            ("USW-Enterprise-24-PoE", "USW-Enterprise-24-PoE", "usw"),
            ("USW-Pro-48-PoE", "USW-Pro-48-PoE", "usw"),
            ("USW-Aggregation", "USW-Aggregation", "usw"),
        };

        foreach (var (model, shortname, type) in switchModels)
        {
            mockDevices.Add(new DiscoveredDevice
            {
                Id = Guid.NewGuid().ToString(),
                Mac = $"aa:bb:cc:dd:{baseIp:x2}:02",
                Name = $"Mock {shortname}",
        ...
        // Gateways (should NOT appear in WiFi Optimizer)
        var gatewayModels = new[] {
            ("UDM-Pro", "UDM-Pro", "udm"),
            ("UDM-SE", "UDM-SE", "udm"),
            ("UDR", "UDR", "udm"),
            ("UCG-Ultra", "UCG-Ultra", "ucg"),
            ("UXG-Pro", "UXG-Pro", "uxg"),
        };

        foreach (var (model, shortname, type) in gatewayModels)
        {
            mockDevices.Add(new DiscoveredDevice
            {
                Id = Guid.NewGuid().ToString(),
                Mac = $"aa:bb:cc:dd:{baseIp:x2}:03",
                Name = $"Mock {shortname}",
        ...
        // SmartPower (should NOT appear - these incorrectly report type=uap)
        var smartPowerModels = new[] {
            ("UP6", "USP-Strip", "uap"),  // Reports as uap but is SmartPower
            ("UP1", "USP-Plug", "uap"),   // Reports as uap but is SmartPower
            ("USP-RPS", "USP-RPS", "uap"),
        };

        foreach (var (model, shortname, type) in smartPowerModels)
        {
            mockDevices.Add(new DiscoveredDevice
            {
                Id = Guid.NewGuid().ToString(),
                Mac = $"aa:bb:cc:dd:{baseIp:x2}:04",
                Name = $"Mock {shortname}",
                Type = DeviceType.SmartPower,  // Correctly classified despite API type
                HardwareType = DeviceType.SmartPower,
                Model = model,
                Shortname = shortname,
                IpAddress = $"192.168.1.{baseIp++}",
                Firmware = "2.0.0",
        ...
        // Cloud Key (should NOT appear)
        mockDevices.Add(new DiscoveredDevice
        {
            Id = Guid.NewGuid().ToString(),
            Mac = $"aa:bb:cc:dd:{baseIp:x2}:05",
            Name = "Mock Cloud Key Gen2 Plus",
            Type = DeviceType.CloudKey,
            HardwareType = DeviceType.CloudKey,
            Model = "UCK-G2-Plus",
            Shortname = "UCK-G2-Plus",
            IpAddress = $"192.168.1.{baseIp++}",
            Firmware = "3.0.0",
            Adopted = true,
            State = 1
        ...
        // Building Bridge (should NOT appear)
        mockDevices.Add(new DiscoveredDevice
        {
            Id = Guid.NewGuid().ToString(),
            Mac = $"aa:bb:cc:dd:{baseIp:x2}:06",
            Name = "Mock Building Bridge",
            Type = DeviceType.BuildingBridge,
            HardwareType = DeviceType.BuildingBridge,
            Model = "UBB",
            Shortname = "UBB",
            IpAddress = $"192.168.1.{baseIp++}",
            Firmware = "1.0.0",
